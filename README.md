...
.\soultuner.ps1 up lite       # Neo4j + Backend + Frontend，无需 GPU
.\soultuner.ps1 doctor        # 绿灯后打开 http://localhost:3003
```

需要长期记忆和元搜索时用 `.\soultuner.ps1 up standard`；需要独立入库 Worker 时用 `.\soultuner.ps1 up full`。联网播放代理是可选本地服务，可用 `.\soultuner.ps1 netease-start` 单独启动。

<details>
<summary>本地开发 / GPU 入库 / 手动分步</summary>

- 本地开发：创建 `music_agent` Conda 环境，安装 Python 与 `web/` 前端依赖后运行 `python startup_all.py`。
- 模型缓存：首次需要 `python scripts/download_models.py` 下载 M2D-CLAP 与 HuggingFace 本地缓存；在线推荐默认只读缓存，不在请求链路下载模型。
- GPU 入库：日常在线推荐不需要 GPU；音频向量提取和批量入库放在 `.\soultuner.ps1 up full` 或 `.\soultuner.ps1 ingest full`。
...
| **大语言模型** | 默认 `dashscope / qwen3.7-plus`，可切换 Gemini / 豆包 / DeepSeek / 本地 Qwen3-4B |
...
| **容器化**     | Docker Compose（Lite/Standard/Full：Neo4j + Backend + Frontend，可选 GraphZep/SearxNG/Worker） |
...
> 意图识别默认使用 `dashscope / qwen3.7-plus`，也支持其他 API 大模型和本地 Qwen3-4B（SGLang 部署）双模式。本地模式下 HyDE 声学描述由独立模块生成。
...
| 命令 | 用途 |
|---|---|
| `.\soultuner.ps1 up lite` | Neo4j + Backend + Frontend，普通体验/演示 |
| `.\soultuner.ps1 up standard` | Lite + GraphZep + SearxNG |
| `.\soultuner.ps1 up full` | Standard + 独立入库 Worker |
| `.\soultuner.ps1 doctor` | 环境体检与下一步建议 |
| `.\soultuner.ps1 test` | 单元测试 |
| `.\soultuner.ps1 mock` | 无外部服务端到端自测 |
| `.\soultuner.ps1 ingest full` | 使用 Full profile 的 Worker 处理入库队列 |
| `.\soultuner.ps1 netease-start` | 可选：启动本地网易云音乐代理 `:3000` |
...
在线推荐只读已挂载的模型缓存；批量歌词标签、音频向量和新歌入库由 Full profile 的 Worker 独立处理，避免日常推荐链路依赖 GPU。
